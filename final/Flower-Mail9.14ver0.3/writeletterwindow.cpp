...
void WriteLetterWindow::judgeEmpty(){
    receivePerson = ui->Lreceiver -> toPlainText();
    copyPerson = ui ->LreceivOther ->toPlainText();
    secretSendPerson = ui -> LreceiverSecert -> toPlainText();
    theme = ui -> LthemInput -> toPlainText();
    if( receivePerson.isEmpty() || copyPerson.isEmpty() || secretSendPerson.isEmpty() || theme.isEmpty()){
...
void WriteLetterWindow::send(){
    ...
    int last = 0;
    QString receiver[ 5 ];
    QString getRec = ui ->Lreceiver -> toPlainText();
    for(int i = 0;i<getRec.length();i++){
        if(getRec[i] == ';'&& count < 5){
            receiver[ count++ ] = getRec.mid( last , i-1-last );
            last = i + 1;
            qDebug()<<"string "<<i<<":"<<receiver[ count-1 ]<<endl;
        }
    }
